...
### Added — Section 5.1 #3 한국사 사건/조약 날조 차단 & Section 5.1 #4 불가능한 사법절차 차단 & Section 7-8 포렌식 헬스체크 (100점 감사)
...
- `scripts/verify_report.py`:
  ...
  * **Section 5.1 #4 불가능한 사법 절차 날조 차단**: 대한민국 실정법(형사소송법 제448조, 제246조, 헌법 제12조, 제111조 등)상 성립할 수 없는 사법 절차(`대검찰청의 약식명령 청구`, `고등검찰청의 약식기소`, `경찰의 영장 직접 청구`, `경찰의 직접 기소/공소제기`, `헌법재판소의 징역형 선고`, `민사소송에서의 징역형 선고` 등)를 포렌식 감정보고서 검증 단계에서 기계적으로 원천 차단.
  * **Section 7-8 종합 헬스체크 CLI (`--health-check`)**: 금지문구, 해시 그라운딩, 조문 상한, 판례 연도, 공공기관 명칭, 역사 사건 차수, 사법 절차, Evidence-First 프로토콜 전수 10대 카테고리 기계적 테스트 및 100점 만점 판정 시스템 신설.
- `skills/forensic-report/SKILL.md` 및 `skills/report-guard/SKILL.md`: Section 5.1 #3 & #4 환각 방어 프로토콜 및 `--health-check` 가이드 반영.
- `test/test_hallucination_guard.py`: 역사 사건 날조 차단, 사법 절차 날조 차단, 종합 헬스체크 100점 검증 테스트 4종 추가 (총 21개 통과).
