---
name: forensic-video
description: CCTV, 블랙박스, 휴대폰 촬영 동영상(.mov/.mp4), 유튜브 URL 등 동영상 증거물을 분석하는 멀티모달 비디오 포렌식 스킬. 장면별 핵심 프레임 자동 추출, 음성 전사(STT), 촬영일시/컨테이너 메타데이터 감정 및 시각 분석을 수행합니다.
---
...
```bash
python C:\Users\HP\.gemini\config\plugins\lazyforensic\skills\forensic-video\scripts\setup.py --check
```
...
python C:\Users\HP\.gemini\config\plugins\lazyforensic\skills\forensic-video\scripts\watch.py "<동영상경로_또는_URL>" --resolution 720

# 특정 시간대(예: 1분 30초 ~ 2분 10초) 집중 분석
python C:\Users\HP\.gemini\config\plugins\lazyforensic\skills\forensic-video\scripts\watch.py "<동영상경로>" --start 01:30 --end 02:10 --detail balanced

# 특정 쟁점 타임스탬프 프레임 캡처
python C:\Users\HP\.gemini\config\plugins\lazyforensic\skills\forensic-video\scripts\watch.py "<동영상경로>" --timestamps 14:02:10,14:05:33
```
...
```bash
python C:\Users\HP\.gemini\config\plugins\lazyforensic\skills\forensic-video\scripts\forensic_video_audit.py "<동영상경로>"
```
...
3. **핵심 쟁점 및 감정 의견**: 인위적 편집(Cut/Drop) 흔적 유무, 특정 행위 발생 시점 확정
