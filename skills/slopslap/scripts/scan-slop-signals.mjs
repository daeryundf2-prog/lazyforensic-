 ...
 * 이 스크립트의 출력은 slopslap 점검 서브에이전트의 입력(후보 목록)이고, 최종 판정은
...
const { AI_SLOP_TAXONOMY } = require(pathToFileURL(path.join(ROOT, 'src/data/aiSlopTaxonomyData.js')).pathname.replace(/^\/([A-Za-z]:)/, '$1'));
