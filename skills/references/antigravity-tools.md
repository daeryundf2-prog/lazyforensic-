...
Defaults: Google Antigravity + Gemini 3.7 Flash (High).
...
`Workspace` is optional. Keep the session UI on Gemini 3.7 Flash (High). For verify lanes pass `Model: "pro"`. Passing `Model` is an agent hint (`hostEnforced=false`).
